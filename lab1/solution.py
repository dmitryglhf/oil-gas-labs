...
def _(mo):
    ...
    В ходе выполнения лабораторной работы была проведена кластеризация профилей добычи нефти 17 разведочных скважин месторождения COSTA с использованием метода Dynamic Time Warping. Анализ данных охватывает 30-летний период помесячных наблюдений с 2020 по 2050 год. Для определения оптимального числа кластеров применялся коэффициент силуэта в сочетании с методом локтя, что позволило выявить три устойчивые группы скважин. Значение Silhouette Score составило 0.32, что соответствует хорошему качеству кластеризации и свидетельствует о наличии реальной структуры в данных.
